...
export default {
  ...
  mounted() {
    ...
    // puppet strings generate **/*.pp --format markdown --out ~/var/tmp/mavdoc.md
    // or
    // puppet strings generate **/*.pp --format json ~/var/tmp/mavdoc.json
    // run those from ~/software/maverick/manifests/maverick-modules
  ...
  methods: {
    ...
    // here you could do a server side lookup
      return [
        {
            label: 'base::python::maverick_python',
            kind: monaco.languages.CompletionItemKind.Property,
            documentation: this.pythondoc+"\n\nmaverick_python\nWhether to install the custom Maverick Python instance.  This should always be true, unless the OS provides a trusted 3.7 instance.",
            insertText: '"base::python::maverick_python": ',
            detail: 'maverick_python Whether to install the custom Maverick Python instance.  This should always be true, unless the OS provides a trusted 3.7 instance."',
            range: range
        },
        {
            label: 'base::python::python_version',
            kind: monaco.languages.CompletionItemKind.Property,
            documentation: this.pythondoc+"\n\npython_version\nThe custom version of Python to compmile and install.",
            insertText: '"base::python::python_version": ',
            detail: "example detail",
            // range: range // not needed?
        }
      ]
    }
  }
